...
labelnames = ['local_ip', 'server_name', 'bind', 'operator', 'tier']

public_ip_gauge = Gauge('public_ip', 'Public IP address for each interface', labelnames)
ip_check_duration = Summary('ip_check_duration_seconds', 'Time taken to check public IP',
                            labelnames)
successful_checks = Counter('successful_ip_checks_total', 'Total number of successful IP checks', labelnames)
...
def execute_call(source_ip: str, server_name: str, bind: str, tier: str, operator: str) -> str:
    with ip_check_duration.labels(local_ip=source_ip, server_name=server_name, bind=bind, operator=operator, tier=tier).time():
        with requests.Session() as session:
            ...
            try:
                ...
                successful_checks.labels(local_ip=source_ip, server_name=server_name, bind=bind, tier=tier,
                                         operator=operator).inc()
                return public_ip
            except requests.Timeout:
                logger.error(f"Timeout error while getting public IP for {source_ip}")
                timeout_errors.labels(local_ip=source_ip, server_name=server_name, bind=bind, tier=tier,
                                      operator=operator).inc()
                return f"Timeout error for {source_ip}"
            except requests.RequestException as e:
                logger.error(f"Failed to get public IP for {source_ip}. Error: {str(e)}")
                other_errors.labels(local_ip=source_ip, server_name=server_name, bind=bind, tier=tier,
                                    operator=operator).inc()
...
def main():
    ...
    while True:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(execute_call, device['addr'], device["server_name"], device["bind"], device["tier"],
